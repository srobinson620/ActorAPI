...
In larger applications we would probably not directly serialise JPS data instead we would have
...
when it flushes it's data do an update.
...
In the createActor method we set the first and last names into an __Actor__ object and save it,
we have not set __actotId__ but it will be autogenerated and back written into the field, the field
...
This is a JPA issue, there is a lot of caching and until the __EntityManager__ (Which is the internal class that 
...
it would need us to take control of the __EntityManager__ or extend the __ActorRepo__ to access the entity manager 
then create a refresh method or use raw JDBC which we are trying to avoid.

This sort of annoyance is why I personally do not like JPA, it's too cunning
...
Unit tests are needed for all the classes in which we have code, so we can leave __ActorRepo__ as all the code is autogenerated.
There is little point in testing the data transfer objects (__Actor__ and __RequestActor__) as they are trivial and contains no function.
...
We will need the instance to test so add it as another class propert.

```
SakilaDAO inst;
```

We are also likely to need an actorId and a couple of names to use so it's easiest to create to create a 
set of constant
...
The first line creates a mock instance of __ActionRepo__, the second and third real instances of __Action__
