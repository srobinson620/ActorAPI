...
We then anable JPA by adding the __@EnableJpaRepositories__ annotaion to the __ActorsApplication__ class.
...
public class SakilaDAO {
    ...
    public Actor createActors(RequestActor actorNoId) {
        ...
        // note that the id has not been set and has been left to be autogenerated
        repo.saveAndFlush(actor);
        // when the actor is saved the autogenerated id will be set into the actor class so when 
        // it is returned it is fully populated.
        // using saveAndFlush rather than save ensures that the database is immediately updated.
        return actor;
    }
}
```
